...
from textclassification import (create_tfidfvectorizer, run_vectorizer,
...
def test_create_tfidfvectorizer(shared):
    test_vectorizer = create_tfidfvectorizer()
    assert isinstance(test_vectorizer, type(shared.target_vectorizer))
    assert test_vectorizer.get_params() == shared.target_vectorizer.get_params()
...
def test_create_model(shared):
    ...
    assert test_model.get_params() == shared.target_model.get_params()
...
def test_create_balanced_model(shared):
    ...
    assert isinstance(test_balanced_model, type(shared.target_balanced_model))
    assert test_balanced_model.get_params() == shared.target_balanced_model.get_params()
